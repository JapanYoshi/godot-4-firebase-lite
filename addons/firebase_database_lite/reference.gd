...
var path : String: get = _get_ref_path  # The (read-only) path this ref points to.
...
func _db_request(parse_response : bool, method, body : String = "") -> Object:
	...
	if _auth and _auth.current_user:
		suffix = await _get_auth_path_suffix()
	else:
	...
	var error = _http.request(
		_db_url + SEP + _path + suffix, [], method, body
	)
	if error == OK:
...
func fetch():
	var result = await _db_request(true, HTTPClient.METHOD_GET)
...
func put(data):
	...
	return await _db_request(true, HTTPClient.METHOD_PUT, body)
...
func update(data):
	var body = json.stringify(data)
	return await _db_request(true, HTTPClient.METHOD_PATCH, body)
...
#
func remove():
	return await _db_request(false, HTTPClient.METHOD_DELETE)
...
#		#TODO: maybe handle existance of current_user differently
	var result = await _auth.current_user.get_id_token()
	if result is FirebaseError:
...
func enable_listener() -> void:
	...
	if !_listener:
		...
		var suffix : String
		if _auth and _auth.current_user:
			suffix = await _get_auth_path_suffix(true)
		else:
